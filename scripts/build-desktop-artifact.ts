...
const DESKTOP_PRODUCT_NAME = desktopPackageJson.productName ?? "CoCo";

const BuildPlatform = Schema.Literals(["mac", "win"]);
...
export function resolveResourceMonitorRustTargets(
  ...
  return [platform === "mac" ? "aarch64-apple-darwin" : "x86_64-pc-windows-msvc"];
}
...
const getDefaultArch = (platform: typeof BuildPlatform.Type) =>
  Effect.succeed(platform === "mac" ? ("arm64" as const) : ("x64" as const));
...
export class DesktopDmgBackgroundSourceMissingError extends Schema.TaggedError<DesktopDmgBackgroundSourceMissingError>()(
  ...
  override get message(): string {
    return `Desktop DMG background source is missing at ${this.sourcePath}`;
  }
}
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // CoCo always passes the user's installed Claude executable to the SDK,
...
// The Apple Silicon app only needs the Darwin arm64 terminal helper.
export function resolveMacFileExclusions(_arch?: typeof BuildArch.Type) {
  return [...MAC_FILE_EXCLUSIONS, "!**/node_modules/node-pty/prebuilds/darwin-x64/**/*"];
}
...
export function resolveWindowsServerAsarIgnoreGlobs(_arch: typeof BuildArch.Type) {
  const unusedArch = "arm64";
...
// The Windows x64 build bundles the matching Linux x64 WSL terminal helper.
export const resolveWslPrebuildArch = (arch: typeof BuildArch.Type): "x64" | undefined =>
  arch === "x64" ? "x64" : undefined;
...
export function resolveFffNativeDependencies(
  platform: typeof BuildPlatform.Type | "linux",
...
export function createStageWorkspaceConfig(input: {
  ...
  const hostOs = platform === "mac" ? "darwin" : "win32";
  const hostCpu = [arch];
  const supportedArchitectures = linuxServerBackend
    ? {
...
function windowsVswherePrerequisiteScript(): string {
  const toolComponents = ["Microsoft.VisualStudio.Component.VC.Tools.x86.x64"];
  const spectreArch = "x64";
  return [
...
export const stageDesktopDmgBackground = Effect.fn("stageDesktopDmgBackground")(function* (
  ...
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const sourcePath = path.join(stageResourcesDir, "dmg", "dmg-background.svg");
  if (!(yield* fs.exists(sourcePath))) {
    return yield* new DesktopDmgBackgroundSourceMissingError({ sourcePath });
  }
  ...
  for (const output of [
    ...
    const targetPath = path.join(stageResourcesDir, "dmg", `dmg-background${output.suffix}.png`);
    yield* runCommand(
      ChildProcess.make(
        {},
      )`sips -s format png -z ${output.height} ${output.width} ${sourcePath} --out ${targetPath}`,
      {
        label: `sips DMG background${output.suffix || "@1x"}`,
        verbose,
      },
    );
  }
...
export function resolveDesktopBuildIconAssets(): DesktopBuildIconAssets {
  return {
    macIconPng: "assets/coco/icon.png",
    windowsIconIco: "assets/coco/icon.ico",
  };
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    // CoCo releases are distributed manually; never infer an upstream update feed.
    publish: null,
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "CoCo captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "CoCo",
  ...
  if (platform === "mac" && target === "dmg") {
    buildConfig.dmg = {
      ...
      title: `${DESKTOP_PRODUCT_NAME} ${version} Installer`,
  ...
  return Effect.succeed(buildConfig);
});
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  yield* fs.copyFile(path.join(repoRoot, "LICENSE"), path.join(stageResourcesDir, "LICENSE"));
  yield* fs.copyFile(
    path.join(repoRoot, "native/libghostty-vt/LICENSE"),
    path.join(stageResourcesDir, "LICENSE-ghostty"),
  );
  ...
  // Keep generated resources in the production directory used by electron-builder.
  ...
  // server packages that remain external to its merged app.asar.
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "CoCo desktop build",
    author: "T3 Tools",
  ...
  //
  // Only Windows unpacks anything; macOS keeps the whole tree inside
  ...
  if (options.platform === "win") {
    yield* validateWindowsPackagedPayload({
      stageDistDir,
      appExecutableName: `${DESKTOP_PRODUCT_NAME}.exe`,
  ...
  if (options.keepStage) {
    const outputPath = yield* Config.string("GITHUB_OUTPUT").pipe(Config.option);
    if (Option.isSome(outputPath) && outputPath.value.trim().length > 0) {
      yield* fs.writeFileString(outputPath.value, `stage_path=${stageRoot}\n`, { flag: "a" });
    }
  }
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  target: Flag.string("target").pipe(
    Flag.withDescription("Artifact target, for example dmg/nsis (env: T3CODE_DESKTOP_TARGET)."),
    Flag.optional,
  ),
  ...
  Command.withDescription("Build a desktop artifact for CoCo."),
