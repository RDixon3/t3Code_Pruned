...
/** Loopback-only binding for every development mode. */
...
export class DevRunnerHostNotProxiableError extends Schema.TaggedError<DevRunnerHostNotProxiableError>()(
  ...
  {
    mode: Schema.Literals(["dev", "dev:web", "dev:server", "dev:desktop"]),
    host: Schema.String,
  },
) {
  override get message(): string {
    return `CoCo development is local-only. Use localhost, 127.0.0.1, or ::1 instead of --host ${this.host}.`;
  }
}
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    if (input.host !== undefined && !isProxiableBindHost(input.host)) {
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  host: Flag.string("host").pipe(
    Flag.withDescription("Loopback host override (localhost, 127.0.0.1, or ::1)."),
