...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  it("uses CoCo artwork for supported desktop builds", () => {
  ...
  it.effect("disables automatic update feeds even when a GitHub repository is configured", () =>
    Effect.gen(function* () {
      const preview = yield* createBuildConfig("mac", "dmg", "0.0.33-pr.8182.1");
      const release = yield* createBuildConfig("mac", "dmg", "0.0.33");
  ...
  it("installs optional natives for macOS arm64, Windows x64, and its WSL backend", () => {
    ...
    assert.deepStrictEqual(
      ...
      {
        supportedArchitectures: { os: ["win32", "linux"], cpu: ["x64"], libc: ["glibc"] },
        nodeLinker: "hoisted",
      },
    );
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      const mac = yield* createBuildConfig("mac", "dmg", "1.2.3");
      const win = yield* createBuildConfig("win", "nsis", "1.2.3", true);
      const winWithoutWslPrebuild = yield* createBuildConfig("win", "nsis", "1.2.3", false);
  ...
  it.effect("reports missing macOS tools and Rust targets before building", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const error = yield* preflightMacDesktopBuild("arm64").pipe(
        ...
        assert.deepStrictEqual(error.rustTargets, ["aarch64-apple-darwin"]);
        assert.include(error.message, "xcode-select --install");
        assert.include(error.message, "rustup target add aarch64-apple-darwin");
      }),
    ),
  );
  ...
  it.effect("rasterizes staged DMG backgrounds at standard and Retina sizes", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const sourcePath = path.join(dmgDir, "dmg-background.svg");
        yield* fs.writeFileString(sourcePath, '<svg xmlns="http://www.w3.org/2000/svg"/>');
        const commands: Array<{ readonly command: string; readonly args: ReadonlyArray<string> }> =
          [];

        yield* stageDesktopDmgBackground(stageResourcesDir, false).pipe(
        ...
        assert.deepStrictEqual(
          ...
          [
            [
              ...
              sourcePath,
              "--out",
              path.join(dmgDir, "dmg-background.png"),
            ],
            [
              "sips",
              "-s",
              ...
              sourcePath,
              "--out",
              path.join(dmgDir, "dmg-background@2x.png"),
            ],
          ],
        );
      }),
  ...
  it.effect("fails clearly when the selected DMG background source is missing", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.include(error.sourcePath, "dmg-background.svg");
      }),
    ),
  );

  it.effect("uses ad-hoc macOS signing and renderer protocols", () =>
    Effect.gen(function* () {
      const config = yield* createBuildConfig("mac", "dmg", "1.2.3");
      ...
      assert.deepStrictEqual(mac.protocols, [{ name: "CoCo", schemes: ["t3code", "t3code-dev"] }]);
    }).pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })))),
  );

  it.effect("preserves CoCo identity and artwork for prerelease macOS builds", () =>
    Effect.gen(function* () {
      const config = yield* createBuildConfig("mac", "dmg", "1.2.3-nightly.20260815.1");

      assert.equal(config.productName, "CoCo");
      assert.equal((config.dmg as Record<string, unknown>).background, "dmg/dmg-background.png");
    }).pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })))),
  );

  it.effect("keeps executable resource editing enabled for unsigned Windows builds", () =>
    Effect.gen(function* () {
      const config = yield* createBuildConfig("win", "nsis", "1.2.3");
  ...
  it("stages the resource monitor as an external executable resource", () => {
    ...
    assert.deepStrictEqual(resolveResourceMonitorRustTargets("mac", "arm64"), [
  ...
  it("packages the WSL server and production dependencies as one compressed runtime", () => {
    ...
    assert.isFalse(bundlesWslRuntime({ arch: "arm64", prebuildPath: "/tmp/pty.node" }));
  ...
  it.effect("rejects Intel macOS and ARM Windows builds before staging binaries", () =>
    Effect.gen(function* () {
      for (const platform of ["mac", "win"] as const) {
        const error = yield* Effect.flip(
          resolveBuildOptions({
            ...
            target: Option.none(),
            arch: Option.some(platform === "mac" ? "x64" : "arm64"),
            buildVersion: Option.none(),
            outputDir: Option.none(),
            skipBuild: Option.none(),
            keepStage: Option.none(),
  ...
  it.effect.each([
    { host: "linux", env: {} },
    { host: "win32", env: { T3CODE_DESKTOP_PLATFORM: "linux" } },
    { host: "darwin", env: { T3CODE_DESKTOP_ARCH: "universal" } },
  ] as const)("rejects removed desktop targets before staging: %j", (input) =>
    Effect.gen(function* () {
      const result = yield* resolveBuildOptions({
        platform: Option.none(),
        target: Option.none(),
        arch: Option.none(),
        buildVersion: Option.none(),
        ...
        Effect.provideService(HostProcessPlatform, input.host),
        Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: input.env }))),
        Effect.flip,
      );
      if (input.host === "linux") assert.instanceOf(result, UnsupportedHostBuildPlatformError);
      else assert.equal(result._tag, "ConfigError");
    }),
  );
