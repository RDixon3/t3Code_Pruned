// @effect-diagnostics nodeBuiltinImport:off - Binding is checked against actual OS interfaces, never DNS.
...
export class LocalServerHostError extends Schema.TaggedError<LocalServerHostError>()(
  "LocalServerHostError",
  { host: Schema.String },
) {
  override get message() {
    return "CoCo only accepts connections from this machine. Use a loopback host.";
  }
}

export const resolveLocalServerHost = (
  host: string | undefined,
): Effect.Effect<string, LocalServerHostError> => {
  const normalized = host?.trim().toLowerCase() || "127.0.0.1";
  if (normalized === "localhost") return Effect.succeed("127.0.0.1");
  if (normalized === "::1" || normalized === "[::1]") return Effect.succeed("::1");
  ...
  return Effect.fail(new LocalServerHostError({ host: host ?? "" }));
};

// WSL NAT requires its exact distro address for the Windows desktop to reach it.
// This exception is passed only from the authenticated private desktop bootstrap.
export const HostInterfaceAddresses = Context.Reference<ReadonlySet<string>>(
  "t3/config/HostInterfaceAddresses",
  {
    defaultValue: () =>
      new Set(
        Object.values(NodeOS.networkInterfaces()).flatMap((entries) =>
          (entries ?? [])
            .filter((entry) => !entry.internal && ["IPv4", "4"].includes(String(entry.family)))
...
export const resolveServerHost = (host: string | undefined, desktopWslHost?: string) =>
  resolveLocalServerHost(host).pipe(
    Effect.catch((error) =>
      Effect.gen(function* () {
        const platform = yield* HostProcessPlatform;
        const environment = yield* HostProcessEnvironment;
        const addresses = yield* HostInterfaceAddresses;
        if (
          platform === "linux" &&
          environment.WSL_DISTRO_NAME?.trim() &&
          host === desktopWslHost &&
          host !== undefined &&
          host !== "0.0.0.0" &&
...
export class ServerConfig extends Context.Service<
  ...
  ServerDerivedPaths & {
    ...
    readonly desktopWslHost?: string | undefined;
