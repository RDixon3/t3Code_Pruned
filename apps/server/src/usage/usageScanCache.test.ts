...
describe("scan cache round trip", () => {
  it("restores records unchanged", () => {
    ...
    original.set("/claude-tail.jsonl", {
      size: 40,
      mtimeMs: 300,
      provider: "claude",
      records: [
        record({ provider: "claude", model: "claude-sonnet-5", dedupeKey: "s:p:claude-sonnet-5" }),
      ],
      tailRecords: [record({ provider: "claude", model: "claude-sonnet-5", dedupeKey: null })],
    ...
    expect(restored.get("/claude-tail.jsonl")).toEqual(original.get("/claude-tail.jsonl"));
