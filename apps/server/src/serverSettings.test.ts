...
it.layer(NodeServices.layer)("server settings", (it) => {
  ...
  it.effect("enables previously used providers from sparse settings files", () =>
    Effect.gen(function* () {
      ...
      yield* fileSystem.writeFileString(
        serverConfig.settingsPath,
        '{"providers":{"cursor":{"apiEndpoint":"https://cursor.example"}}}',
      );
      yield* recordProviderUsage("cursor");

      const settings = yield* serverSettings.getSettings;
      assert.isTrue(settings.providers.cursor.enabled);
      assert.equal(settings.providers.cursor.apiEndpoint, "https://cursor.example");
    }).pipe(Effect.provide(makeServerSettingsLayer())),
  );
  ...
  it.effect("preserves existing provider instances without explicit enabled flags", () =>
    Effect.gen(function* () {
      ...
      yield* fileSystem.writeFileString(
        serverConfig.settingsPath,
        '{"providerInstances":{"cursor_work":{"driver":"cursor","config":{}},"cursor_unused":{"driver":"cursor","config":{}}}}',
      );
  ...
  it.effect("preserves explicit provider disables in existing settings files", () =>
    Effect.gen(function* () {
      ...
      yield* fileSystem.writeFileString(
        serverConfig.settingsPath,
        '{"providers":{"cursor":{"enabled":false}},"providerInstances":{"cursor":{"driver":"cursor","enabled":false,"config":{}}}}',
      );
      yield* recordProviderUsage("cursor");
  ...
  it.effect("persists explicit disables after a provider has been used", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue((yield* serverSettings.getSettings).providers.cursor.enabled);
      ...
      const raw = yield* fileSystem.readFileString(serverConfig.settingsPath);
      // @effect-diagnostics-next-line preferSchemaOverJson:off
      assert.isFalse(JSON.parse(raw).providers.cursor.enabled);
    }).pipe(Effect.provide(makeServerSettingsLayer())),
  );
  ...
  it.effect("keeps optional providers disabled after a new installation writes settings", () =>
    Effect.gen(function* () {
      ...
      assert.isFalse(next.providers.cursor.enabled);
      const cursor = next.providerInstances[ProviderInstanceId.make("cursor")];
      assert.isDefined(cursor);
      assert.isFalse(resolveProviderInstanceEnabled(cursor));
      ...
      assert.isFalse(persisted.providers.cursor.enabled);
      assert.isUndefined(persisted.providerInstances.cursor.enabled);
    }).pipe(Effect.provide(makeServerSettingsLayer())),
  );
    ...
    Effect.gen(function* () {
      ...
      const next = yield* serverSettings.updateSettings({
        providers: {
          ...
          cursor: {
            binaryPath: "  /opt/homebrew/bin/cursor  ",
            apiEndpoint: "  http://127.0.0.1:4096  ",
          },
        },
      });
