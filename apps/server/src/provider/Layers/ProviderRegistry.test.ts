...
function mockSpawnerLayer(
  ...
  ).pipe(Layer.merge(Layer.succeed(HostProcessPlatform, "linux")));
}
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsModule.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("refreshes a Codex instance catalog and preserves other instances", () =>
        Effect.gen(function* () {
          const codexDriver = ProviderDriverKind.make("codex");
          const workCodexDriver = ProviderDriverKind.make("codex");
          const codexInstanceId = ProviderInstanceId.make("codex");
          const workCodexInstanceId = ProviderInstanceId.make("codex-work");
          const codexRefreshCalls = yield* Ref.make(0);
          const workCodexRefreshCalls = yield* Ref.make(0);
          const codexProvider = {
            instanceId: codexInstanceId,
            driver: codexDriver,
            status: "ready",
          ...
          const failedWorkCodexProvider = {
            instanceId: workCodexInstanceId,
            driver: workCodexDriver,
            status: "error",
            enabled: true,
            installed: true,
            auth: { status: "unknown" },
            checkedAt: "2026-06-10T00:00:00.000Z",
            version: "1.0.0",
            message: "Failed to refresh Codex models.",
            models: [],
            slashCommands: [],
            skills: [],
          } as const satisfies ServerProvider;
          const recoveredWorkCodexProvider = {
            ...failedWorkCodexProvider,
            status: "ready",
            auth: { status: "authenticated" },
            checkedAt: "2026-06-10T00:01:00.000Z",
            message: "Codex model catalog refreshed.",
            models: [
              {
                slug: "gpt-5",
                name: "GPT-5",
          ...
          const catalogSnapshot = yield* Ref.make<ServerProvider>(recoveredWorkCodexProvider);
          const instances = [
            {
              instanceId: codexInstanceId,
              driverKind: codexDriver,
            ...
            },
            {
              ...
              snapshot: {
                resolveMaintenance: () =>
                  Effect.succeed(
                    makeManualOnlyProviderMaintenanceCapabilities({
                      provider: workCodexDriver,
                      packageName: null,
                    }),
                  ),
                getSnapshot: Effect.succeed(failedWorkCodexProvider),
                refresh: Ref.update(workCodexRefreshCalls, (count) => count + 1).pipe(
          ...
          yield* Effect.gen(function* () {
            ...
            assert.strictEqual(
              initialProviders.find((provider) => provider.instanceId === workCodexInstanceId)
                ?.status,
              "error",
            );
            ...
            assert.deepStrictEqual(
              recoveredProviders.find((provider) => provider.instanceId === workCodexInstanceId)
                ?.models,
              recoveredWorkCodexProvider.models,
            );
            assert.deepStrictEqual(
            ...
            assert.deepStrictEqual(
              changedProviders.find((provider) => provider.instanceId === workCodexInstanceId)
                ?.models,
          ...
          assert.strictEqual(yield* Ref.get(workCodexRefreshCalls), 2);
        }),
      );
        ...
        () =>
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(cursorProvider?.message, "Cursor is disabled in CoCo settings.");
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in CoCo settings.");
        }),
      );
    });
