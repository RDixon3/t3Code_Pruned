...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("does not expose the removed T3 Connect routes", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      for (const route of [
        ...
        const response = yield* HttpClient.post(route);
        assert.equal(response.status, 404);
      }
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("provider setup rejects sign-in changes from read-only clients", () =>
    Effect.gen(function* () {
      let authCalls = 0;
      yield* buildAppUnderTest({
        layers: {
