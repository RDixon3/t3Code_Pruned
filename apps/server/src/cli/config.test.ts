...
import { HostInterfaceAddresses, deriveServerPaths } from "../config.ts";
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect.each(["flags", "environment", "bootstrap"] as const)(
    "rejects non-loopback hosts supplied through %s",
    (source) =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "coco-local-host-test-" });
        const fd =
          source === "bootstrap"
        ...
        const error = yield* resolveServerConfig(
          {
            ...
            host: source === "flags" ? Option.some("0.0.0.0") : Option.none(),
            baseDir: Option.some(baseDir),
            cwd: Option.none(),
            devUrl: Option.none(),
            noBrowser: Option.some(true),
            bootstrapFd: Option.fromUndefinedOr(fd),
            ...
            Layer.mergeAll(
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: source === "environment" ? { T3CODE_HOST: "0.0.0.0" } : {},
                }),
              ),
              NetService.layer,
            ),
  ...
  it.effect.each(["bootstrap", "flags", "environment", "empty-token", "web-mode"] as const)(
    "allows WSL interface binding only through a desktop bootstrap: %s",
    (source) =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "coco-wsl-host-test-" });
        const fd = yield* openBootstrapFd(
          makeDesktopBootstrap({
            host: "172.24.1.2",
            t3Home: baseDir,
            desktopBootstrapToken: source === "empty-token" ? "" : "desktop-bootstrap-token",
          }),
        );
        const result = yield* resolveServerConfig(
          {
            mode: Option.some(source === "web-mode" ? "web" : "desktop"),
            port: Option.some(3773),
            host: source === "flags" ? Option.some("172.24.1.2") : Option.none(),
            baseDir: Option.some(baseDir),
            cwd: Option.none(),
            devUrl: Option.none(),
            noBrowser: Option.some(true),
          ...
          Effect.provide(
            Layer.mergeAll(
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: source === "environment" ? { T3CODE_HOST: "172.24.1.2" } : {},
                }),
              ),
              NetService.layer,
            ),
          ),
          Effect.provideService(HostProcessEnvironment, { WSL_DISTRO_NAME: "Ubuntu" }),
          Effect.provideService(HostInterfaceAddresses, new Set(["172.24.1.2"])),
          Effect.result,
        );
        const platform = yield* HostProcessPlatform;
        expect(result._tag === "Success").toBe(source === "bootstrap" && platform === "linux");
      }),
  );
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "coco-bootstrap-config-" });
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
