 ...
 * shared T3 home. Pairing is only available on this machine.
 */
...
class NoRunningServerError extends Schema.TaggedError<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      ...
      "Start the CoCo desktop app or the local development server.",
    ].join("\n");
  }
}

/** The URL a local development browser should pair through. */
...
/** Confirm the recorded local server still answers with a valid descriptor. */
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      const notes: Array<string> = ["This URL is only reachable from this machine."];
      const pairingBaseUrl = resolveDirectPairingBaseUrl(target.state);
      if (!isLoopbackHost(new URL(pairingBaseUrl).hostname)) {
        return yield* new ServerConfig.LocalServerHostError({
          host: new URL(pairingBaseUrl).hostname,
        });
      }
