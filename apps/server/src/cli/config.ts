...
const modeFlag = Flag.choice("mode", ServerConfig.RuntimeMode.literals).pipe(
  Flag.withDescription("Runtime mode. Both modes accept local connections only."),
  Flag.optional,
);
...
const hostFlag = Flag.string("host").pipe(
  Flag.withDescription("Loopback host to bind (127.0.0.1, localhost, or ::1)."),
  Flag.optional,
);
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    const desktopWslHost =
      mode === "desktop" &&
      desktopBootstrapToken &&
      Option.isNone(normalizedFlags.host) &&
      env.host === undefined
        ? bootstrap?.host
        : undefined;
    const host = yield* ServerConfig.resolveServerHost(
      Option.getOrElse(
        resolveOptionPrecedence(
