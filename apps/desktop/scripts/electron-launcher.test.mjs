...
describe("electron development launcher", () => {
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/CoCo (Dev).app",
      "CoCo (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "CoCo (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/CoCo (Dev).app/Contents/MacOS/CoCo (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/CoCo (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/CoCo (Dev).app/Contents/MacOS/Electron'",
    );
    assert.notInclude(script, "node_modules/electron");
  });

  it("declares why the macOS app needs protected access", () => {
    const values = resolveMacBundleInfoPlistStrings("CoCo (Dev) Launcher");

    assert.equal(
      values.NSScreenCaptureUsageDescription,
      "CoCo captures the active window when you use the snapshot shortcut.",
    );
    assert.equal(
      values.NSDocumentsFolderUsageDescription,
      "CoCo reads project files you open in the desktop app.",
    );
  });

  it("ad-hoc signs the complete development app bundle", () => {
    assert.deepEqual(resolveMacCodeSignArguments("/runtime/CoCo (Dev).app"), [
      "--force",
      "--deep",
      "--sign",
      "-",
      "--timestamp=none",
      "/runtime/CoCo (Dev).app",
    ]);
  });

  // oxlint-disable-next-line t3code/no-global-process-runtime -- This standalone launcher test requires POSIX file modes, which Windows does not support.
  ...
  it("uses CoCo artwork with separate development and production icon caches", () => {
    ...
    assert.match(development.sourceIconPath, /assets[\\/]coco[\\/]icon\.png$/);
    assert.equal(development.generatedIconPath, "/runtime/coco-icon-dev.icns");
    assert.match(production.sourceIconPath, /assets[\\/]coco[\\/]icon\.png$/);
    assert.equal(production.generatedIconPath, "/runtime/coco-icon-prod.icns");
  });
});
