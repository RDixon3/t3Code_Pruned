// This file mostly exists because we want dev mode to say "CoCo (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "CoCo (Dev)" : "CoCo (Alpha)";
const APP_BUNDLE_ID = isDevelopment
  ? `io.github.rdixon3.coco.dev.${devBundleIdSuffix || "local"}`
  : "io.github.rdixon3.coco";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["t3code-dev"] : ["t3code"];
const LAUNCHER_VERSION = 20;
const appIconPngPath = NodePath.join(repoRoot, "assets", "coco", "icon.png");
...
export function resolveMacLauncherIconPaths(runtimeDir, development = isDevelopment) {
  return {
    sourceIconPath: appIconPngPath,
    generatedIconPath: NodePath.posix.join(
      runtimeDir,
      development ? "coco-icon-dev.icns" : "coco-icon-prod.icns",
    ),
  };
}
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSDocumentsFolderUsageDescription: "CoCo reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the CoCo name.
