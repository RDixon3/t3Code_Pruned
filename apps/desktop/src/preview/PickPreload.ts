// @effect-diagnostics globalDate:off globalTimers:off cryptoRandomUUID:off - This isolated Electron preload does not run inside an Effect runtime.
...
import { ELEMENT_CONTEXT_ATTRIBUTE, normalizeElementContext } from "./ElementContext.ts";
...
async function captureElement(element: Element): Promise<PickedElementPayload> {
  ...
  element.setAttribute(ELEMENT_CONTEXT_ATTRIBUTE, marker);
  try {
    const context = normalizeElementContext(
      await withCaptureTimeout(
        ipcRenderer.invoke(ELEMENT_CONTEXT_CHANNEL, marker),
    ...
    if (element.getAttribute(ELEMENT_CONTEXT_ATTRIBUTE) === marker) {
      element.removeAttribute(ELEMENT_CONTEXT_ATTRIBUTE);
    }
  }
  return {
    ...base,
