...
import { isTrustedDesktopIpcSender, type IpcSenderEvent } from "./TrustedIpcSender.ts";

export interface DesktopIpcInvokeEvent extends IpcSenderEvent {}

interface DesktopIpcSyncEvent extends IpcSenderEvent {
  returnValue: unknown;
}
          ...
          try: () => {
            ...
            ipcMain.handle(channel, (event, raw) => {
              if (!isTrustedDesktopIpcSender(event)) {
                throw new Error("Desktop request was rejected.");
              }
              return runPromise(
                Effect.gen(function* () {
                  yield* Effect.annotateCurrentSpan({ channel });
