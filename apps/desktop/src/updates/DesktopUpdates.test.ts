...
describe("DesktopUpdates", () => {
  it.effect("does not check or subscribe to updates without a packaged feed", () => {
    const harness = makeHarness({ appUpdateConfig: "" });
  ...
  it.effect("recovers download state after an unexpected setup failure", () => {
    const harness = makeHarness({
      downloadUpdate: Effect.die(new Error("download setup failed")),
    });
      ...
      const harness = makeHarness({
        downloadUpdate: Effect.suspend(() => {
          ...
          return downloadCalls === 1
            ? Deferred.succeed(actionStarted, undefined).pipe(Effect.andThen(Effect.never))
            : Effect.void;
        }),
      });
