...
export class DesktopBackendConfiguration extends Context.Service<
  DesktopBackendConfiguration,
  {
    ...
    >;
    // Bind only the desktop-authorized WSL interface (or localhost in mirrored mode).
...
> {
  ...
  // Use the exact local WSL interface when localhost forwarding is unavailable.
  // Shared/mirrored networking uses loopback; never bind every interface.
  const bootstrap = {
    mode: "desktop" as const,
    noBrowser: true,
    port: input.port,
...
export const make = Effect.gen(function* () {
  ...
  // Primary and WSL backends share the renderer bootstrap token.
  const sharedInputs = getOrCreateBootstrapToken.pipe(
    Effect.map((bootstrapToken) => ({ bootstrapToken }) satisfies SharedBootstrapInput),
  );
