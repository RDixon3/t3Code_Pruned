...
// Resolve only endpoints owned by running desktop instances. Keeping this a
// live pool read revokes a removed WSL instance without reloading the renderer.
export const readEndpoints = Effect.gen(function* () {
  const pool = yield* DesktopBackendPool;
  const instances = yield* pool.list;
  const endpoints = yield* Effect.forEach(instances, (instance) =>
    Effect.gen(function* () {
      const snapshot = yield* instance.snapshot;
      if (!snapshot.desiredRunning) return null;
      const config = yield* instance.currentConfig;
      return Option.isSome(config) && Option.isNone(config.value.preflightFailure)
        ? { id: instance.id, origin: config.value.httpBaseUrl }
        : null;
    }),
  );
  const active = endpoints.filter((endpoint) => endpoint !== null);
  return {
    primaryOrigin: active.find((endpoint) => endpoint.id === PRIMARY_INSTANCE_ID)?.origin ?? null,
    backendOrigins: active.map((endpoint) => endpoint.origin),
  };
});
