...
describe("accessibleWindowElementTree", () => {
  ...
  it("uses null child bounds when a provider reports only the root origin", async () => {
    const tree = await accessibleWindowElementTree(
      {
        role: "window",
  ...
  it("keeps child bounds after observing real position variation", async () => {
    const tree = await accessibleWindowElementTree(
      {
        role: "window",
        bounds: { x: 0, y: 0, width: 800, height: 600 },
...
describe("findAccessibleWindow", () => {
  ...
  it("keeps exact title matching exactly", () => {
    expect(
