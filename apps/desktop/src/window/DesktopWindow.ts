...
import { trustDesktopIpcSender } from "../ipc/TrustedIpcSender.ts";
...
export function isSameOriginRendererNavigation(input: {
  ...
  try {
    ...
    // Custom schemes have the same opaque URL.origin as file/data URLs in Node.
    return (
...
export const make = Effect.gen(function* () {
  ...
  > {
    ...
    const revokeIpcSender = trustDesktopIpcSender(window.webContents, applicationUrl);
    ...
    window.webContents.on("will-attach-webview", (event, webPreferences, params) => {
      ...
      // A renderer may attach only the bundled annotation preload, never a
      // renderer-supplied preload with application privileges.
      webPreferences.preload = environment.path.join(
        environment.dirname,
