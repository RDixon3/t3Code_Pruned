...
class PrimaryEnvironmentRemoteTargetUnsupportedError extends Schema.TaggedError<PrimaryEnvironmentRemoteTargetUnsupportedError>()(
  "PrimaryEnvironmentRemoteTargetUnsupportedError",
  {},
) {
  override get message(): string {
    return "CoCo web development connects only to this computer. Open its localhost address.";
  }
}
...
export function readPrimaryEnvironmentTarget(): PrimaryEnvironmentTarget {
  ...
  for (const [raw, protocols] of [
    [target.target.httpBaseUrl, ["http:", "https:"]],
    [target.target.wsBaseUrl, ["ws:", "wss:"]],
  ] as const) {
    const url = new URL(raw);
    if (
      !protocols.includes(url.protocol as never) ||
      url.username ||
      url.password ||
      (target.source !== "desktop-managed" && !isLoopbackHostname(url.hostname))
    )
      throw new PrimaryEnvironmentRemoteTargetUnsupportedError({});
  }
  return target;
}
