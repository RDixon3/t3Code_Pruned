...
describe("environmentBootstrap", () => {
  it("rejects a remote configured endpoint before descriptor or auth requests", () => {
    vi.stubEnv("VITE_HTTP_URL", "https://remote.example.com");
    expect(readPrimaryEnvironmentTarget).toThrow(
      "CoCo web development connects only to this computer",
    );
  });
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  it("uses https descriptor urls when the primary environment uses wss", async () => {
    vi.stubEnv("VITE_HTTP_URL", "https://localhost:3201");
    vi.stubEnv("VITE_WS_URL", "wss://localhost:3201");
    ...
    expect(resolvePrimaryEnvironmentHttpUrl("/.well-known/t3/environment")).toBe(
      "https://localhost:3201/.well-known/t3/environment",
    );
  });
  ...
  it("derives the websocket url when only VITE_HTTP_URL is configured", async () => {
    ...
    expect(getPrimaryKnownEnvironment()?.target).toEqual({
      httpBaseUrl: "https://localhost:3201/",
      wsBaseUrl: "wss://localhost:3201/",
    });
  });
