...
export function ServerUpdatesAction({
  ...
  const eligible = targets.filter(
    (target) => target.selfUpdate === "desktop-managed" && target.desktopAppUpdate,
  );
...
export function ServerUpdateAction({
  ...
  if (selfUpdate === null) {
    return (
      <span className="text-muted-foreground text-xs">
        Update and restart CoCo. For local development, restart the web app and backend from the
        same checkout.
      </span>
    );
  }
