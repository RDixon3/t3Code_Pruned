...
import { WizardHeader, WizardPanel, WizardPopup, WizardSteps } from "../ui/wizard";
...
export function WelcomeWizard({
  ...
  return (
    <Dialog open disablePointerDismissal onOpenChange={(_, event) => event.cancel()}>
      <WizardPopup bottomStickOnMobile={false} showCloseButton={false}>
        <WizardHeader
          title="Set up CoCo"
          identity={
  ...
  return (
    <>
      ...
      <p className="mt-2.5 text-sm leading-relaxed text-muted-foreground">
        Choose the local environments where you want to set up agents and projects.
      </p>
      <fieldset className="mt-5 space-y-2">
        ...
        {environments.map((environment) => (
          ...
          >
            ...
            <span className="min-w-0 flex-1 text-sm font-medium">{environment.label}</span>
