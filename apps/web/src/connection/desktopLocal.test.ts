...
import {
  ...
  isDesktopBootstrapEndpointPair,
...
describe("trusted desktop endpoint pairs", () => {
  const bootstrap = {
    id: "primary",
    label: "WSL",
  ...
  it("accepts an exact local WSL bootstrap and normalized trailing slash", () => {
    expect(
      isDesktopBootstrapEndpointPair(
        { httpBaseUrl: bootstrap.httpBaseUrl + "/", wsBaseUrl: bootstrap.wsBaseUrl + "/" },
        [bootstrap],
      ),
    ).toBe(true);
  });
  it("rejects other private addresses, ports, credentials, and missing bootstraps", () => {
    for (const httpBaseUrl of [
      "http://172.27.0.100:3201",
      "http://172.27.0.99:3202",
      "http://user@172.27.0.99:3201",
    ]) {
      expect(
        isDesktopBootstrapEndpointPair({ httpBaseUrl, wsBaseUrl: bootstrap.wsBaseUrl }, [
          bootstrap,
        ]),
      ).toBe(false);
    }
    expect(isDesktopBootstrapEndpointPair(bootstrap, [])).toBe(false);
  });
});
