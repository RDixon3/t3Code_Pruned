...
/** Trust exact endpoints supplied by the desktop's own backend pool, never saved remote profiles. */
export function isDesktopBootstrapEndpointPair(
  input: { readonly httpBaseUrl: string; readonly wsBaseUrl: string },
  bootstraps: ReadonlyArray<DesktopEnvironmentBootstrap>,
): boolean {
  const normalize = (value: string, protocols: ReadonlyArray<string>): string | null => {
    try {
  ...
  const http = normalize(input.httpBaseUrl, ["http:", "https:"]),
    ws = normalize(input.wsBaseUrl, ["ws:", "wss:"]);
  return (
    http !== null &&
    ws !== null &&
    bootstraps.some(
      (entry) =>
        ...
        normalize(entry.httpBaseUrl, ["http:", "https:"]) === http &&
        normalize(entry.wsBaseUrl, ["ws:", "wss:"]) === ws,
    )
  );
}
