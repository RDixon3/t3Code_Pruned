(() => {
  ...
  const BUILT_IN_THEME_MODES = Object.fromEntries(
    Object.entries(BUILT_IN_THEME_PALETTES).map(([id, palettes]) => [id, Object.keys(palettes)]),
  );
  ...
  const isStoredCustomTheme = (value) => {
    ...
    return Object.entries(value.variants).every(
      ([appearance, colors]) =>
        isThemeAppearance(appearance) && (appearance === value.appearance || isRecord(colors)),
    );
  };
  const findCustomTheme = (themeId) => {
    if (RESERVED_THEME_IDS.includes(themeId)) return null;
    try {
      const parsed = JSON.parse(window.localStorage.getItem(CUSTOM_THEMES_STORAGE_KEY) ?? "null");
      if (!Array.isArray(parsed)) return null;
      return parsed.find((value) => isStoredCustomTheme(value) && value.id === themeId) ?? null;
    } catch {
      return null;
    }
  };
  ...
  try {
    ...
    const themeId = storedTheme === null ? "" : (LEGACY_THEME_IDS[storedTheme] ?? storedTheme);
    ...
    const storedAppearanceMode = window.localStorage.getItem(THEME_APPEARANCE_MODE_STORAGE_KEY);
    ...
    try {
      const halvesRaw = JSON.parse(window.localStorage.getItem(THEME_HALVES_STORAGE_KEY) ?? "null");
    ...
    const customColors =
      effectiveIsThemed && effectiveCustomTheme
        ? effectiveThemeMode === effectiveCustomTheme.appearance
          ...
          : (effectiveCustomTheme.variants?.[effectiveThemeMode] ?? effectiveCustomTheme.colors)
        : null;
