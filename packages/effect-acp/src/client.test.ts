  ...
  it.effect("reports idle child termination and rejects later requests and notifications", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        // A signal-terminated process may expose a failed exitCode on Windows.
        // Validate the platform result rather than requiring a numeric exit status.
        yield* handle.exitCode.pipe(
          Effect.match({
            onSuccess: (code) => {
              assert.equal(error._tag, "AcpProcessExitedError");
              if (error._tag === "AcpProcessExitedError") assert.equal(error.code, code);
            },
            onFailure: () => {
              assert.equal(error._tag, "AcpTransportError");
              if (error._tag === "AcpTransportError") {
                assert.equal(error.operation, "read-process-exit-status");
              }
            },
          }),
        );
        assert.ok(error._tag === "AcpProcessExitedError" || error._tag === "AcpTransportError");
