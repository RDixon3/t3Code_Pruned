...
import { agentMethods, schemaRoots, retainSchemas } from "./retainedProtocol.ts";
...
const generateSchemas = Effect.fn("generateSchemas")(function* (skipDownload: boolean) {
  ...
  const retainedEntries = retainSchemas(generatedEntries, schemaRoots);
  const prelude = [
  ...
  const schemaOutput = [
    ...
    'import * as Schema from "effect/Schema";',
    "",
    [...retainedEntries.values()].join("\n\n"),
    "",
  ].join("\n");

  const metaOutput = [
    ...prelude,
    `export const AGENT_METHODS = ${yield* encodeAgentMethods(Object.fromEntries(Object.entries(upstreamMeta.agentMethods).filter(([name]) => agentMethods.has(name))))} as const;`,
    "",
  ...
  yield* Effect.log(
    `Generated ${retainedEntries.size} retained ACP schemas from ${CURRENT_SCHEMA_RELEASE}`,
  );
