...
import { isLoopbackConnectionUrl } from "./local.ts";
import type {
...
export const make = Effect.gen(function* () {
  ...
  // Old catalogs still decode; remote entries stay dormant instead of reconnecting.
  const initialEntries = new Map<EnvironmentId, ConnectionCatalogEntry>();
  for (const target of persistedTargets) {
    if (target._tag === "PrimaryConnectionTarget") {
      if (
        isLoopbackConnectionUrl(target.httpBaseUrl, ["http:", "https:"]) &&
        isLoopbackConnectionUrl(target.wsBaseUrl, ["ws:", "wss:"])
      )
        initialEntries.set(target.environmentId, { target, profile: Option.none() });
      ...
      if (
        ...
        isLoopbackConnectionUrl(profile.value.httpBaseUrl, ["http:", "https:"]) &&
        isLoopbackConnectionUrl(profile.value.wsBaseUrl, ["ws:", "wss:"])
      )
        initialEntries.set(target.environmentId, { target, profile });
    }
  }
  const entries =
  ...
  const start = Effect.gen(function* () {
    ...
    yield* Effect.forEach(
      ...
      (entry) =>
        acquireSupervisor(entry.target.environmentId).pipe(
  ...
  const register = Effect.fn("EnvironmentRegistry.register")(function* (
    ...
    if (
      ...
      !isLoopbackConnectionUrl(registration.profile.httpBaseUrl, ["http:", "https:"]) ||
      !isLoopbackConnectionUrl(registration.profile.wsBaseUrl, ["ws:", "wss:"])
    ) {
      return yield* new Persistence.ConnectionPersistenceError({
        operation: "register-connection",
        message: "Remote environments are not supported in CoCo.",
      });
    }
