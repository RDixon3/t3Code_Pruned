...
const PREPARED: PreparedConnection = {
  ...
  socketUrl: "ws://127.0.0.1:3201/ws",
...
const BEARER_PROFILE = new BearerConnectionProfile({
  ...
  httpBaseUrl: "http://127.0.0.1:3203",
  wsBaseUrl: "ws://127.0.0.1:3203",
});
...
describe("EnvironmentRegistry", () => {
  it.effect("keeps legacy remote entries dormant without connecting or deleting them", () =>
    Effect.gen(function* () {
      const remote = new PrimaryConnectionTarget({
        environmentId: EnvironmentId.make("legacy-remote"),
        label: "Legacy",
        httpBaseUrl: "https://remote.example.test",
        wsBaseUrl: "wss://remote.example.test",
      });
      const harness = yield* makeHarness([remote]);
      yield* Effect.gen(function* () {
        ...
        expect((yield* Ref.get(harness.storedTargets)).has(remote.environmentId)).toBe(true);
