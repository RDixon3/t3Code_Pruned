...
describe("ClientSettings sidebar", () => {
  it("ignores retired sidebar switches while retaining current preferences", () => {
    const decoded = decodeClientSettings({
...
describe("provider enabled defaults", () => {
  it("ignores retired provider and telemetry settings without losing current configuration", () => {
    const decoded = decodeServerSettings({
      providers: {
        cursor: { enabled: true },
        grok: { enabled: true },
      ...
      observability: { otlpTracesUrl: "https://collector.example/traces" },
    });
    expect(Object.keys(decoded.providers)).toEqual(["codex", "claudeAgent", "cursor"]);
    expect(decoded.providers.cursor.enabled).toBe(true);
    expect(decoded).not.toHaveProperty("observability");
  });
  ...
  it("resolves instance enabled state with explicit false winning", () => {
    ...
    expect(resolveProviderInstanceEnabled({ driver: cursor, config: {} })).toBe(false);
    ...
    expect(resolveProviderInstanceEnabled({ driver: cursor, enabled: true, config: {} })).toBe(
      true,
    );
    ...
    expect(resolveProviderInstanceEnabled({ driver: cursor, config: { enabled: true } })).toBe(
      true,
    );
    // Conflicting flags: the explicit false wins, whichever side it is on.
    expect(
      resolveProviderInstanceEnabled({ driver: cursor, enabled: true, config: { enabled: false } }),
    ).toBe(false);
    expect(
